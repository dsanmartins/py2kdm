...
from kdm_architecture_review.gui.architecture_tree_view import ArchitectureTreeView
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.architecture_tree = ArchitectureTreeView()
        self.component_list = QListWidget()
        self.relationship_list = QListWidget()

        self.left_tabs = QTabWidget()
        self.left_tabs.addTab(self.architecture_tree, "Architecture")
    ...
    def _signals(self):
        self.architecture_tree.element_selected.connect(
            self._architecture_element_selected
        )
    ...
    def export_reviewed(self):
        ...
        if report.get("summary", {}).get("forbidden", 0) > 0:
            ans = QMessageBox.question(
                self,
                "Forbidden findings",
                "The reviewed architecture has FORBIDDEN findings. Export anyway?",
            )
    ...
    def _refresh_all(self):
        self._refresh_lists()
        self.architecture_tree.render_model(self.session.model)
    ...
    def _refresh_lists(self):
        ...
        for c in self.session.components:
            ...
            stereotype = c.get("stereotype_name") or c.get("role", "?")
            item = QListWidgetItem(
                f"{marker} {c.get('name','Component')} [{stereotype}]"
            )
            item.setData(Qt.ItemDataRole.UserRole, c.get("id"))
            self.component_list.addItem(item)

        for r in self.session.containment_relationships + self.session.relationships:
            marker = "✓" if r.get("materialize", True) is not False else "✗"
            item = QListWidgetItem(
                f"{marker} {r.get('type','relationship')} "
                f"[{r.get('relationship_level','?')}]"
            )
            item.setData(Qt.ItemDataRole.UserRole, r.get("id"))
            self.relationship_list.addItem(item)

    def _architecture_element_selected(self, element_id):
        component = self.session.get_component(element_id)
        if component:
            self.properties_panel.show_component(element_id)
            return

        # Subsystems and control loops are currently read-only in the property
        # panel. They are shown in the tree and graph to preserve hierarchy.
        node = self.session.get_node(element_id)
        if node:
            self.statusBar().showMessage(
                f"Selected architecture element: {node.get('name', element_id)}"
            )

    def _component_selected(self, current, previous):
        if current:
            self.properties_panel.show_component(
                current.data(Qt.ItemDataRole.UserRole)
            )

    def _relationship_selected(self, current, previous):
        if current:
            self.properties_panel.show_relationship(
                current.data(Qt.ItemDataRole.UserRole)
            )
